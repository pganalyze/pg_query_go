...
func (node OnConflictExpr) Fingerprint(ctx FingerprintContext, parentFieldName string) {
	...
	if len(node.ArbiterElems.Items) > 0 {
		...
		node.ArbiterElems.Fingerprint(&subCtx, "ArbiterElems")
	...
	if node.ArbiterWhere != nil {
		subCtx := FingerprintSubContext{}
		node.ArbiterWhere.Fingerprint(&subCtx, "ArbiterWhere")
	...
	if len(node.ExclRelTlist.Items) > 0 {
		subCtx := FingerprintSubContext{}
		node.ExclRelTlist.Fingerprint(&subCtx, "ExclRelTlist")
	...
	if len(node.OnConflictSet.Items) > 0 {
		subCtx := FingerprintSubContext{}
		node.OnConflictSet.Fingerprint(&subCtx, "OnConflictSet")
	...
	if node.OnConflictWhere != nil {
		subCtx := FingerprintSubContext{}
		node.OnConflictWhere.Fingerprint(&subCtx, "OnConflictWhere")
